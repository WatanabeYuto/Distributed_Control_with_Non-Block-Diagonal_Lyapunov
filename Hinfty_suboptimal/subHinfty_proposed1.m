function [gamma_opt,K_opt,P_opt] = subHinfty_proposed1(params,flag)
    ...
    Theta = blkdiag(rho*M,zeros(n),zeros(dimce));
    LMI = [LMI_1 + Theta <= - eps * eye(size(Theta,1)), Q_e >=eps *eye(dim), Q_e*M + M*Q_e >= eta*M, eta >=eps];
